...
Modify resources/public/index.html to include the newly created *tailwind-base.css* and *tailwind-component.css*, which are relatively small, and remove *tailwind.min.css*. These two css files relatively small, since most of the meat is found in *tailwind-utilities.css*, which we don't have to include if we use *garden-mower* to extract the class attributes we are interested in.
